...
def build_callback_list(custom_tensorboard_callback: BaseCallback) -> CallbackList:
    """Build the callback stack for PPO training.

    Args:
        custom_tensorboard_callback: The local checkpoint and metric callback.
...
class SpeculativeDecodingEnv(gym.Env):
    ...
    def _prepare_for_next_topk_cycle(self, accepted_hidden_state_base, next_token_sampled):
        """Prime the next speculative cycle and track drafter usage.
    ...
    def build_dataset_manifest(self, total_timesteps: int, question_file: str) -> dict[str, Any]:
        """Build dataset-style logging metadata for one LTD depth phase.
        ...
        return {
            ...
            "base_model_path": args.base_model_path,
            "ea_model_path": args.ea_model_path,
            "rl_token_model_path": args.rl_token_model_path,
            "rl_checkpoint_path": args.rl_checkpoint_path,
            "fixed_total_token": 60,
            "max_draft_depth": 12,
...
if __name__ == '__main__':
    ...
    # SB3 PPO with an MLP policy is more stable and typically faster on CPU.
    device = "cpu"
    ...
    final_model_path = promote_best_checkpoint(
        ...
        final_model_name=os.path.join(
            args.save_path,
            "ppo_speculative_decoder_controller_v1_single_action",
        ),
        model_rl=model_rl,
    )
