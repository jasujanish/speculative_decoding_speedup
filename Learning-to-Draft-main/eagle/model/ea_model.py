...
def require_optional_model(model_cls, model_name, dyn_depth_type):
    """Validate that an optional dynamic-depth module is available.

    Args:
        model_cls: Imported class object or ``None``.
        model_name: Module name in ``eagle.model``.
        dyn_depth_type: Requested dynamic depth strategy.

    Returns:
        The imported class when available.

    Raises:
        ImportError: If the optional module is not present in this repo.
    """
    if model_cls is None:
        raise ImportError(
            f"dyn_depth_type='{dyn_depth_type}' requires eagle.model.{model_name}, "
            "but that file is not present in this repository snapshot."
        )
    return model_cls
    ...
    def __init__(
        ...
        if use_eagle3:
            if dyn_depth_type=="ddd":
                print("model ddd")
                model_cls = require_optional_model(Modelddd, "cnets_ddd", dyn_depth_type)
                self.ea_layer=model_cls(config, bias=bias, total_tokens=total_token, depth=depth, top_k=top_k,
                                  threshold=threshold, path=base_model_name_or_path,load_emb=True)
            elif dyn_depth_type=="svip":
                print("model svip")
                model_cls = require_optional_model(Modelsvip, "cnets_svip", dyn_depth_type)
                self.ea_layer=model_cls(config, bias=bias, total_tokens=total_token, depth=depth, top_k=top_k,
                                  threshold=threshold, path=base_model_name_or_path,load_emb=True)
            elif dyn_depth_type=="gammatune":
                print("model gammatune")
                model_cls = require_optional_model(Modelgamma, "cnets_gamma", dyn_depth_type)
                self.ea_layer=model_cls(config, bias=bias, total_tokens=total_token, depth=depth, top_k=top_k,
                                  threshold=threshold, path=base_model_name_or_path,load_emb=True)
            elif dyn_depth_type=="c2t":
                model_cls = require_optional_model(Modelc2t, "cnets_c2t", dyn_depth_type)
                self.ea_layer=model_cls(config, bias=bias, total_tokens=total_token, depth=depth, top_k=top_k,
                                  threshold=threshold, path=base_model_name_or_path,load_emb=True)
                self.c2t=load_c2t_model()
            elif dyn_depth_type=="disco":
                self.disco=load_disco_model()
                model_cls = require_optional_model(Modeldisco, "cnets_disco", dyn_depth_type)
                self.ea_layer=model_cls(config, bias=bias, total_tokens=total_token, depth=depth, top_k=top_k,
                                  threshold=threshold, path=base_model_name_or_path,load_emb=True)
            elif dyn_depth_type=="specplus":
                self.spec_plus=load_spec_plus_model()
                model_cls = require_optional_model(Modelspecplus, "cnets_spec_plus", dyn_depth_type)
                self.ea_layer=model_cls(config, bias=bias, total_tokens=total_token, depth=depth, top_k=top_k,
                                  threshold=threshold, path=base_model_name_or_path,load_emb=True)
            else:
