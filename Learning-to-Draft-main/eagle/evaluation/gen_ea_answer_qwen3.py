...
def get_model_answers(
    ...
    model = EaModel.from_pretrained(
        ...
        use_dyn_len=args.use_dyn_depth,
        use_dyn_token=args.use_dyn_token,
        use_rl=args.use_rl,
    ...
    for question in tqdm(questions):
        ...
        for i in range(num_choices):
            ...
            choices.append({"index": i, "turns": turns, "idxs": idxs, "new_tokens": new_tokens, "wall_time": wall_time,"pre_len_times":pre_len_times,"pre_num":pre_num,"dtimes":dtimes,"dyn_tokens":dyn_tokens,"dyn_depths":dyn_depths})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help="Predefined Qwen3 plus AngelSlim Eagle3 model pair.",
    )
    parser.add_argument(
        "--ea-model-path",
        type=str,
        default="/home/v-jiebzhang/hf_models/Qwen3-14B_eagle3",
        help="The path to the weights. This can be a local folder or a Hugging Face repo ID.",
    )
    parser.add_argument("--base-model-path", type=str, default="/home/v-jiebzhang/hf_models/Qwen3-14B",
                        help="1")
    parser.add_argument(
        "--load-in-8bit", action="store_false", help="Use 8-bit quantization"
    )
    parser.add_argument("--model-id", type=str, default="qwen3")
    parser.add_argument(
        "--bench-name",
        type=str,
        default="gsm8k",
    ...
    parser.add_argument(
        "--question-end", type=int,default=80, help="A debug option. The end index of questions."
    )
    ...
    parser.add_argument("--use_dyn_token", action="store_true")
    parser.add_argument("--token_model",type=str,default="")
    ...
    filename_without_zip=""
    args.model_id = args.model_id + "-temperature"+str(args.temperature)+"token"+str(args.total_token)+"depth"+str(args.depth)+"choices"+str(args.num_choices)+"dav"
    if args.depth_model:
        depth_model_basename = os.path.basename(args.depth_model)
        filename_without_zip = os.path.splitext(depth_model_basename)[0]
        args.model_id+=filename_without_zip
    if args.token_model: 

        token_model_basename = os.path.basename(args.token_model)
        filename_without_zip = os.path.splitext(token_model_basename)[0]
        args.model_id+=filename_without_zip
