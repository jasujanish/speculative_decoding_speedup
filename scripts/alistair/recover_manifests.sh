#!/usr/bin/env bash
# Post-hoc recovery for Alistair's supervised-depth dataset manifests.
# Default Modal profile. Cells: 8b/14b × 100k × {base,titan}.
#
...
LOG="$REPO_ROOT/scripts/alistair/recover_manifests.log"
...
echo "############################################################"
echo "# Manifest recovery (Alistair) started at $(date -Iseconds)"
echo "############################################################"
...
recover_one() {
  ...
  local staging="$REPO_ROOT/.manifest_staging_${label}_${variant}_${tk_label}"
  ...
  echo ""
  echo "=========================================================="
  echo "=== [$(date -Iseconds)] recover $label $variant $tk_label"
  echo "=========================================================="
...
recover_one qwen3_8b  8b  100000 100k base
recover_one qwen3_8b  8b  100000 100k titan
recover_one qwen3_14b 14b 100000 100k base
recover_one qwen3_14b 14b 100000 100k titan

echo ""
echo "############################################################"
echo "# MANIFEST RECOVERY (Alistair) COMPLETE at $(date -Iseconds)"
echo "############################################################"
